function [f,error,x,FES,dim] = newDBG(x,fun_num,change_instance,FES,num_runs,dim,fun_opts)
...
%         weight = bsxfun(@times,weight,power(max_weight,10));
...
%                 tmp = fsphere(((x-o(i,:))/lamda(i))*M1)
%                 tmp = C*tmp
%                 tmp = tmp/fmax(i)
%                 tmp = tmp + h(i)
%                 tmp = weight(i)*tmp
%                 f = f+weight(i)*(C*fsphere(((x-o(i,:))/lamda(i))*M1)/fmax(i)+h(i));
            end
...
%         %             x_max = bsxfun(@times, x_max, M);
%         lamda = sigma*(bounds(2)-bounds(1))/(domain1(2)-domain1(1))*lamda;
%         for i=1:10
%             fmax = fsphere(x_max*M(:,:,i));
%             %                 f_sphere = fsphere((bsxfun(@minus,x,o(i,:))/lamda(i))*M(:,:,i))
%             %                 f_hat = C*f_sphere
%             %                 f_hat = f_hat/fmax(i)
%             %                 f_hat = f_hat+h(i)
%             %                 f_hat = weight(:,i).*f_hat
%             f = f + weight(:,i).*(C*fsphere((bsxfun(@minus,x,o(i,:))/lamda(i))*M(:,:,i))/fmax+ h(i));
%         end
        switch(fun_num)
            case 2
                x_max = ones(1,dim)*domain1(2);
